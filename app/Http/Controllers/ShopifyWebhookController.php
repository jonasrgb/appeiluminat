...
class ShopifyWebhookController extends Controller
{
    ...
    public function createWebhookOnlyFields()
    {
        $shopifyStore = 'eiluminat.myshopify.com'; // doar domeniul, fără https://
        $accessToken  = env('ACCESS_TOKEN_ADMIN_EILUMINAT');
        ...
        "webhookSubscription" => [
            "callbackUrl" => "https://coolify.lustreled.ro/api/webhooks/shopify/update",
            "format" => "JSON",
            "metafieldNamespaces" => ["dont"],
            "filter" => "metafields.namespace:dont AND metafields.key:trigger2 AND metafields.value:true"
    ...
    public function getAllWebhooks2()
    {
        $shopUrl = env('SHOPIFY_SHOP_EILUMINAT_URL');
        $accessToken = env('ACCESS_TOKEN_ADMIN_EILUMINAT');

        try {
            $graphqlUrl = "https://{$shopUrl}/admin/api/2025-10/graphql.json"; // aliniază cu docul curent
            $query = <<<'GRAPHQL'
            query ListWebhooks {
            webhookSubscriptions(first: 250) {
                edges {
            ...
            $webhooks = array_map(function ($edge) {
                $n = $edge['node'] ?? [];
                return [
                    'id'         => isset($n['id']) ? (int) str_replace('gid://shopify/WebhookSubscription/', '', $n['id']) : null,
                    'gid'        => $n['id'] ?? null,
                    'topic'      => $n['topic'] ?? null,
                    'created_at' => $n['createdAt'] ?? null,
                    'format'     => $n['format'] ?? null,
                    'uri'        => $n['uri'] ?? null, // HTTPS / ARN / pubsub://
                ];
            }, $edges);
