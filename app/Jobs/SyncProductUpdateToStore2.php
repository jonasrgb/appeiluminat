...
class SyncProductUpdateToStore2 implements ShouldQueue
{
...
public function handle()
{
    ...
    Log::info("✅ Sincronizare completă pentru produsul de pe Powerled {$parentProductId}");
}
    ...
    {
        return new Client([
            'base_uri' => "https://" . env('STORE_URL_POWERLED') . "/admin/api/2024-07/graphql.json",
            'headers' => [
            ...
            'input' => [
                'setQuantities' => [
                    [
                        ...
                        'locationId' => 'gid://shopify/Location/' . env('LOCATION_POWERLEDS'),
    ...
    public function uploadProductMedia(string $productId, array $imageUrls, Client $client, string $productTitle)
    {
        ...
        $mediaArray = array_map(fn($url) => [
            'alt' => "{$productTitle} powerleds.ro",  // Dynamic alt text using product title and 'powerleds.ro'
