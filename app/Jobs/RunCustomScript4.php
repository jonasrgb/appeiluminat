...
class RunCustomScript4 implements ShouldQueue
{
    ...
    {
        $this->collectionId = "684142657910";  // ID-ul colecției
        $this->shopDomain = "iluminat-industrial.myshopify.com";
    }
    ...
    public function handle()
    {
        try {
            ...
            \Log::info('Products removal job completed successfully industrial');
        } catch (\Exception $e) {
    ...
    private function removeProductsFromCollection(array $productIds, $collectionId, $client)
    {
        ...
        foreach ($chunks as $index => $chunk) {
            try {
                ...
                \Log::info('Products chunk removed from collection industrial', [
                    'chunk' => $index + 1,
                    'of' => $totalChunks,
                    'count' => count($chunk),
                ]);
