...
class RunCustomScript3 implements ShouldQueue
{
    ...
    public function handle()
    {
        try {
            ...
            \Log::info('Products removal job completed successfully eiluminat');
        } catch (\Exception $e) {
    ...
    private function removeProductsFromCollection(array $productIds, $collectionId, $client)
    {
        ...
        foreach ($chunks as $index => $chunk) {
            try {
                ...
                \Log::info('Products chunk removed from collection eiluminat', [
                    'chunk' => $index + 1,
                    'of' => $totalChunks,
                    'count' => count($chunk),
                ]);
    ...
    /**
     * Execute removal for a chunk of product ids (<=250).
     */
